...
public class Swea_4727_견우와직녀_실패 {
	...
	static void dfs(int nowi,int nowj,boolean[][][] visited,int skill,int time) {
		...
		visited[nowi][nowj][skill] = true;
		D[nowi][nowj][skill] = Math.min(time, D[nowi][nowj][skill]);
		for(int d=0;d<4;d++) {
			int nexti = nowi+di[d];
			int nextj = nowj+dj[d];
			
			if(nexti<0 || nextj<0 || nexti>=N || nextj>=N || visited[nexti][nextj][skill] ) {
				continue;
			}
			
			if(map[nexti][nextj]==1) {
				dfs(nexti,nextj,visited,skill,time+1);
			}
			if(map[nexti][nextj]==0 && skill==0 ) {
				int num=time%M;
				num=time-num+M;
				dfs(nexti,nextj,visited,skill+1,num);
			}
